...
private:
    ...
    void InitializeButtons() {
        // Register GPIO mapping with ButtonManager (logical -> physical) for menu navigation.
        ButtonManager::GetInstance().Init({
            {ButtonId::MENU_UP, BUTTON_UP_GPIO},
            {ButtonId::MENU_DOWN, BUTTON_DOWN_GPIO},
            {ButtonId::SELECT, BUTTON_SELECT_GPIO},
            {ButtonId::BACK, BUTTON_START_GPIO},
        });
        ...
        button_up_.OnClick([log_btn]() {
            log_btn("Button UP", BUTTON_UP_GPIO);
            ButtonManager::GetInstance().Trigger(ButtonId::MENU_UP);
        });
        button_down_.OnClick([log_btn]() {
            log_btn("Button DOWN", BUTTON_DOWN_GPIO);
            ButtonManager::GetInstance().Trigger(ButtonId::MENU_DOWN);
        });
        button_select_.OnClick([log_btn]() {
            log_btn("Button SELECT", BUTTON_SELECT_GPIO);
            ButtonManager::GetInstance().Trigger(ButtonId::SELECT);
        });
        button_start_.OnClick([log_btn]() {
            log_btn("Button START", BUTTON_START_GPIO);
            ButtonManager::GetInstance().Trigger(ButtonId::BACK);
        });

        // Optional: keep logging for unused arrows

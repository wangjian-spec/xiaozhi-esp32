...
bool g_ui_debug_logging_enabled = APP_UI_DEBUG_LOG_ENABLED_DEFAULT != 0;

} // namespace

bool UiDebugLoggingEnabled() {
	return g_ui_debug_logging_enabled;
}

void SetUiDebugLoggingEnabled(bool enabled) {
	g_ui_debug_logging_enabled = enabled;
}
...
void PrintSceneLoaded(const char* scene_id, uint16_t scene_index, Widget* root, bool has_public) {
    if (!UiDebugLoggingEnabled()) return;

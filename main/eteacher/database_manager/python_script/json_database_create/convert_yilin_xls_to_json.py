...
SOURCE_XLS_PATH = BOOK_DIR / "译林版小学单词表（3-6年级）.xls"
...
OUTPUT_JSON_PATH = BOOK_DIR / "译林版小学单词表（3-6年级）.json"
PUBLISHER = "译林版"
EXPECTED_HEADERS = ["教材", "课本", "单元", "单词", "中文", "音标"]
...
def normalize_unit_label(value: Any) -> str:
    unit = normalize_text(value)
    if not unit:
        return ""

    numeric_candidate = unit.removesuffix(".0")
    if numeric_candidate.isdigit():
        return f"Unit {numeric_candidate}"

    lowered = unit.casefold()
    if lowered.startswith("unit "):
        suffix = unit[5:].strip()
        return f"Unit {suffix}" if suffix else "Unit"
...
def build_source(book_label: str, unit_label: str) -> str:
    parts = [PUBLISHER]
    if book_label:
...
def parse_records(rows: list[list[Any]]) -> tuple[list[dict[str, Any]], int]:
    ...
    for row_number, row in enumerate(rows[header_row_index + 1 :], start=header_row_index + 2):
        ...
        unit_label = normalize_unit_label(row[mapping["单元"]])
        ...
        records.append(
            {
                ...
                "word_meaning": {
                    ...
                    "source": build_source(book_label, unit_label),
                    "word_tag": None,
                },
                "word_form": [],
                "word_example": [],
...
def build_output(records: list[dict[str, Any]], unique_word_count: int) -> dict[str, Any]:
    ...
    return {
        "metadata": {
            "source_file": str(SOURCE_XLS_PATH),

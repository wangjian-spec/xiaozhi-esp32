...
class WordsJsonService:
    ...
    def format_payload_for_display(self, json_path: Path, payload: dict[str, Any]) -> str:
            ...
            for table_name in RELATED_TABLES:
                ...
                if isinstance(columns, list) and columns:
                    for column in columns:
                        ...
                        parts = [f"name={column_name}", f"type={column_type}"]
                        if column.get("pk"):
                            parts.append("pk=1")
                        if column.get("notnull"):
                            parts.append("notnull=1")
                        if column.get("dflt_value") is not None:
                            parts.append(f"default={column.get('dflt_value')}")
                        lines.append(f"  - {', '.join(parts)}")
                else:
                    lines.append("  - 未提供 columns 信息")
                lines.append("")
        else:
            lines.extend(["未提供 schema", ""])

        lines.append("records 全量内容:")
        if not records:
            lines.append("[]")
        else:
    ...
    def flatten_records_for_table(self, payload: dict[str, Any]) -> tuple[list[str], list[dict[str, str]]]:
        records = payload.get("records")
        if not isinstance(records, list):
            return [], []
        ...
        for index, record in enumerate(records, start=1):
            ...
            self._flatten_record_value(flat_row, "", record)
    ...
    def _flatten_record_value(self, target: dict[str, str], prefix: str, value: Any) -> None:
        if isinstance(value, dict):
            ...
            for key in sorted(value):
                ...
                self._flatten_record_value(target, next_prefix, value.get(key))
            return
        ...
        if isinstance(value, list):
            ...
            if all(isinstance(item, dict) for item in value):
                ...
                for field_name in field_names:
                    ...
                    values = [self._scalar_to_text(item.get(field_name)) for item in value]
                    target[column_name] = " | ".join(values)
                return
            target[prefix or "value"] = " | ".join(self._scalar_to_text(item) for item in value)
            return

        target[prefix or "value"] = self._scalar_to_text(value)
    ...
    def import_json_to_database(self, payload: dict[str, Any], output_db_path: Path) -> dict[str, int]:
        ...
        with sqlite3.connect(output_db_path) as conn:
            ...
            normalized_rows = self._normalize_rows_for_import(
                records,
                self._read_max_ids(conn),
                self._load_existing_import_state(conn, schema),
                self._meaning_source_column(schema),
            )
    ...
    def _load_existing_import_state(
        self,
        ...
        meanings_by_word_id: dict[int, list[dict[str, Any]]] = defaultdict(list)

        for row in conn.execute('SELECT * FROM "word" ORDER BY id').fetchall():
            row_dict = dict(row)
            word_key = self._word_lookup_key(row_dict.get("word"))
            if word_key:
                words_by_key[word_key] = row_dict

        for row in conn.execute('SELECT * FROM "word_meaning" ORDER BY word_id, id').fetchall():
            row_dict = dict(row)
            word_id = self._as_int(row_dict.get("word_id"))
            if word_id is None:
                continue
            meanings_by_word_id[word_id].append(row_dict)

        return {
            "words_by_key": words_by_key,
            "meanings_by_word_id": meanings_by_word_id,
            "meaning_source_column": self._meaning_source_column(schema),
        }

    @staticmethod
    def _meaning_source_column(schema: dict[str, SchemaTable]) -> str:
        word_meaning_columns = schema["word_meaning"].column_names
        return "source" if "source" in word_meaning_columns else "image"

    def _normalize_rows_for_import(
        self,
        records: list[Any],
        existing_max_ids: dict[str, int] | None = None,
        existing_import_state: dict[str, Any] | None = None,
        meaning_source_column: str = "source",
    ) -> dict[str, list[dict[str, Any]]]:
        max_ids = dict(existing_max_ids or {table_name: 0 for table_name in RELATED_TABLES})
        existing_words_by_key: dict[str, dict[str, Any]] = dict((existing_import_state or {}).get("words_by_key") or {})
        existing_meanings_by_word_id: dict[int, list[dict[str, Any]]] = dict(
            (existing_import_state or {}).get("meanings_by_word_id") or {}
        )
        ...
        claimed_meaning_ids: dict[int, set[int]] = defaultdict(set)
        ...
        for index, record in enumerate(records, start=1):
            ...
            self._align_meaning_source_field(meaning, meaning_source_column)

            word_key = self._word_lookup_key(word.get("word"))
            existing_word = existing_words_by_key.get(word_key) if word_key else None
            if existing_word is not None:
                word_id = self._as_int(existing_word.get("id"))
                if word_id is None:
                ...
                word_id = self._ensure_row_id(word, "word", next_ids)
                if word_key:
                    existing_words_by_key[word_key] = dict(word)

            meaning_id = self._resolve_meaning_id(
                meaning,
                word_id,
                existing_meanings_by_word_id.get(word_id, []),
                claimed_meaning_ids[word_id],
                next_ids,
                meaning_source_column,
            )
    ...
    @staticmethod
    def _word_lookup_key(word: Any) -> str:
        return str(word or "").strip().casefold()

    def _align_meaning_source_field(self, meaning: dict[str, Any], meaning_source_column: str) -> None:
        if meaning_source_column == "source":
            if "source" not in meaning and "image" in meaning:
                meaning["source"] = meaning.pop("image")
            return
        if "image" not in meaning and "source" in meaning:
            meaning["image"] = meaning.pop("source")

    def _resolve_meaning_id(
        self,
        meaning: dict[str, Any],
        word_id: int,
        existing_meanings: list[dict[str, Any]],
        claimed_meaning_ids: set[int],
        next_ids: dict[str, int],
        meaning_source_column: str,
    ) -> int:
        desired_signature = self._meaning_lookup_signature(meaning, word_id, meaning_source_column)

        for existing_meaning in existing_meanings:
            existing_id = self._as_int(existing_meaning.get("id"))
            if existing_id is None or existing_id in claimed_meaning_ids:
                continue
            existing_signature = self._meaning_lookup_signature(
                existing_meaning,
                word_id,
                meaning_source_column,
            )
            if existing_signature != desired_signature:
                continue
            meaning["id"] = existing_id
            claimed_meaning_ids.add(existing_id)
            return existing_id
        ...
        return self._ensure_row_id(meaning, "word_meaning", next_ids)

    def _meaning_lookup_signature(
        self,
        meaning: dict[str, Any],
        word_id: int,
        meaning_source_column: str,
        ...
        return (
            word_id,
            self._as_int(meaning.get("stage")),
            str(meaning.get("pos") or "").strip(),
            str(meaning.get("meaning_en") or "").strip(),
            str(meaning.get("meaning_zh") or "").strip(),
            str(meaning.get(meaning_source_column) or "").strip(),
            str(meaning.get("word_tag") or "").strip(),
        )
...
class WordsJsonManagerWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        preview_import_json_button = QPushButton("预览选中")
        preview_import_json_button.clicked.connect(self.preview_selected_json_file)
        import_buttons_layout.addWidget(preview_import_json_button)
        ...
        preview_group = QGroupBox("JSON 内容预览")
        preview_layout = QVBoxLayout(preview_group)
        preview_button_layout = QHBoxLayout()
        choose_preview_json_button = QPushButton("选择并预览 JSON")
        choose_preview_json_button.clicked.connect(self.choose_preview_json_path)
        preview_button_layout.addWidget(choose_preview_json_button)
        refresh_preview_button = QPushButton("刷新当前预览")
        refresh_preview_button.clicked.connect(self.refresh_preview_json_file)
        preview_button_layout.addWidget(refresh_preview_button)
        preview_layout.addLayout(preview_button_layout)

        self.preview_path_label = QLabel("当前预览: 未选择文件")
        preview_layout.addWidget(self.preview_path_label)

        self.preview_summary_label = QLabel("records 数量: 0，字段列数: 0")
        ...
        self.preview_tabs.addTab(self.preview_table, "表格预览")

        self.preview_edit = QPlainTextEdit()
        self.preview_edit.setReadOnly(True)
        self.preview_edit.setPlaceholderText("选择 JSON 文件后，这里会显示所有记录和字段内容")
        self.preview_tabs.addTab(self.preview_edit, "文本预览")
        ...
        self.import_json_list.itemSelectionChanged.connect(self._handle_import_json_selection_changed)
    ...
    def choose_preview_json_path(self) -> None:
        selected, _ = QFileDialog.getOpenFileName(
            self,
            "选择要预览的 JSON 文件",
            str(self.current_import_json_paths[0].parent if self.current_import_json_paths else self.export_json_path.parent),
        ...
        existing_index = next(
            (index for index, path in enumerate(self.current_import_json_paths) if str(path) == str(selected_path)),
            None,
        )
        if existing_index is None:
            self.current_import_json_paths.append(selected_path)
            self._sync_path_widgets()
            existing_index = len(self.current_import_json_paths) - 1
            self.log(f"已添加预览文件到列表: {selected_path}")

        self.import_json_list.setCurrentRow(existing_index)
        self._preview_json_file(selected_path)
    ...
    def preview_selected_json_file(self, silent: bool = False) -> None:
        current_item = self.import_json_list.currentItem()
        if current_item is None:
            self._clear_preview()
            if not silent:
                QMessageBox.information(self, "提示", "请先在列表中选中要预览的 JSON 文件")
            return
        self._preview_json_file(Path(current_item.text()), silent=silent)

    def refresh_preview_json_file(self) -> None:
        current_item = self.import_json_list.currentItem()
        if current_item is None:
            QMessageBox.information(self, "提示", "请先选择一个 JSON 文件再刷新预览")
            return
        self._preview_json_file(Path(current_item.text()))
    ...
    def _sync_path_widgets(self) -> None:
        ...
        self.import_json_list.blockSignals(True)
        ...
        self.import_json_list.blockSignals(False)

        if self.current_import_json_paths:
            self.import_json_list.blockSignals(True)
            self.import_json_list.setCurrentRow(0)
            self.import_json_list.blockSignals(False)
            first_path = self.current_import_json_paths[0]
            if first_path.exists():
                self._preview_json_file(first_path, silent=True)
            else:
                self._clear_preview()
        else:
            self._clear_preview()
    ...
    def _handle_import_json_selection_changed(self) -> None:
        self.preview_selected_json_file(silent=True)

    def _preview_json_file(self, json_path: Path, silent: bool = False) -> None:
        try:
            payload = self.service.load_json(json_path)
            preview_text = self.service.format_payload_for_display(json_path, payload)
            columns, row_maps = self.service.flatten_records_for_table(payload)
        except Exception as exc:
            self.preview_path_label.setText(f"当前预览: {json_path}")
            self.preview_summary_label.setText("records 数量: 0，字段列数: 0")
        ...
        self.preview_path_label.setText(f"当前预览: {json_path}")
        self._update_preview_table(columns, row_maps)
        self.preview_edit.setPlainText(preview_text)
        if not silent:
            self.log(f"已加载 JSON 预览: {json_path}")

    def _clear_preview(self) -> None:
        self.preview_path_label.setText("当前预览: 未选择文件")
        self.preview_summary_label.setText("records 数量: 0，字段列数: 0")
        self.preview_table.clear()
        self.preview_table.setRowCount(0)
        self.preview_table.setColumnCount(0)
        self.preview_edit.setPlainText("选择 JSON 文件后，这里会显示所有记录和字段内容")

    def _update_preview_table(self, columns: list[str], row_maps: list[dict[str, str]]) -> None:
        self.preview_table.clear()
        self.preview_table.setRowCount(len(row_maps))
        self.preview_table.setColumnCount(len(columns))
        self.preview_table.setHorizontalHeaderLabels(columns)

        for row_index, row_map in enumerate(row_maps):
            for column_index, column_name in enumerate(columns):
                item = QTableWidgetItem(row_map.get(column_name, ""))
                item.setToolTip(row_map.get(column_name, ""))
                self.preview_table.setItem(row_index, column_index, item)

        self.preview_summary_label.setText(
            f"records 数量: {len(row_maps)}，字段列数: {len(columns)}"
        )
        if columns:

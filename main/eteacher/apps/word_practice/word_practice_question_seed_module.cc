...
bool OpenSeedDictionaryDb(int stage_index, sqlite3 **out_db, std::string *opened_db_path) {
	...
	*out_db = nullptr;
	...
	const std::string db_path = eteacher::database_manager::DiscoverDictionaryDbPath(kTag, stage_index);
	if (db_path.empty()) {
...
bool LoadVocabularySeedForWord(const word_practice::SelectedWord &selected_word,
	...
	bool loaded = false;
	{
		...
		if (!PrepareSeedQueryContext(db, &query_context)) {
			CloseSeedDb(db, "LoadVocabularySeedForWord.prepare_failed");
			ESP_LOGW(kTag, "prepare single seed query context failed path=%s", opened_db_path.c_str());
			return false;
		}
		loaded = LoadVocabularySeed(db, query_context, selected_word, out_seed);
	}
	CloseSeedDb(db, "LoadVocabularySeedForWord");
	WP_SEED_LOGW(kTag,

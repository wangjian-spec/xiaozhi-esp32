...
帮我编写一个软键盘模块，文件编写在app_service\tool\soft_keyboard.cc文件中，该模块可以实现通过调用epd_manager显示一个可设置大小和位置的软键盘具有如下功能；
...
5.选中的字母用圆角矩形圈上
6.成员函数主要包括ShowKeyboard，CloseKeyboard等
8.墨水屏局部刷新逻辑为通过tool\combo_click.cc连击按键逻辑判断，当输出单击或者组合连击事件时，按照返回的单击事件或连击事件一次性移动矩形选中框，只执行一次墨水屏局部刷新逻辑，切换页面或确定输入时执行一次刷新逻辑；

选项卡模块
1.选项卡用于给app作为参数设置或者分类展示用途；其它app可以创建子类扩展功能，创建该对象并使用，该模块写在eteacher\app_service\tool中；
2.该模块可以配置若干个选项，每个选项有自己的属性页，通过上下左右键可以选择不同的选项，未被选中的选项为白底黑字，选中的选项变成黑底白字；
3.可以通过参数设置选项卡的位置和大小，上部为选项区，根据配置行列数绘制等宽等高的网格，下部为属性区，用于显示当前选项的属性内容；
4.按C键确定选择该选项，在属性区显示该选型属性内容，此时不能选择选项，可以编辑属性内容；按B键返回选择选项卡状态，不显示属性内容，不可以编辑属性内容，此时可以选择选项；
5.属性区也可以通过设置参数划分网格；

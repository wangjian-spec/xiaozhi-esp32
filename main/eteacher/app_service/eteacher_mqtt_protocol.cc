...
#define TAG "ETEACHER_MQTT"

EteacherMqttProtocol::EteacherMqttProtocol() {
    event_group_handle_ = xEventGroupCreate();

    // Initialize reconnect timer
    esp_timer_create_args_t reconnect_timer_args = {
        .callback = [](void* arg) {
            EteacherMqttProtocol* protocol = (EteacherMqttProtocol*)arg;
            auto& app = AppService::GetInstance();
            if (app.GetDeviceState() == kEteacherStateIdle) {
...
EteacherMqttProtocol::~EteacherMqttProtocol() {
    ESP_LOGI(TAG, "EteacherMqttProtocol deinit");
...
bool EteacherMqttProtocol::Start() {
    return StartMqttClient(false);
}

bool EteacherMqttProtocol::StartMqttClient(bool report_error) {
    if (mqtt_ != nullptr) {
    ...
    mqtt_->OnDisconnected([this]() {
        ...
        ESP_LOGI(TAG, "MQTT disconnected, schedule reconnect in %d seconds", ETEACHER_MQTT_RECONNECT_INTERVAL_MS / 1000);
        esp_timer_start_once(reconnect_timer_, ETEACHER_MQTT_RECONNECT_INTERVAL_MS * 1000);
    });
    ...
    mqtt_->OnMessage([this](const std::string& topic, const std::string& payload) {
        ...
        if (strcmp(type->valuestring, "hello") == 0) {
            ...
            if (session_id == nullptr || session_id_ == session_id->valuestring) {
                ...
                AppService::GetInstance().Schedule([this, alive]() {
                    if (*alive) {
                        CloseAudioChannel();
                    }
                });
...
bool EteacherMqttProtocol::SendText(const std::string& text) {
...
bool EteacherMqttProtocol::SendAudio(std::unique_ptr<AudioStreamPacket> packet) {
...
void EteacherMqttProtocol::CloseAudioChannel() {
    {
        std::lock_guard<std::mutex> lock(channel_mutex_);
        udp_.reset();
    }
...
bool EteacherMqttProtocol::OpenAudioChannel() {
    ...
    session_id_ = "";
    xEventGroupClearBits(event_group_handle_, ETEACHER_MQTT_PROTOCOL_SERVER_HELLO_EVENT);
    ...
    // 等待服务器响应
    EventBits_t bits = xEventGroupWaitBits(event_group_handle_, ETEACHER_MQTT_PROTOCOL_SERVER_HELLO_EVENT, pdTRUE, pdFALSE, pdMS_TO_TICKS(10000));
    if (!(bits & ETEACHER_MQTT_PROTOCOL_SERVER_HELLO_EVENT)) {
...
std::string EteacherMqttProtocol::GetHelloMessage() {
...
void EteacherMqttProtocol::ParseServerHello(const cJSON* root) {
    ...
    local_sequence_ = 0;
    remote_sequence_ = 0;
    xEventGroupSetBits(event_group_handle_, ETEACHER_MQTT_PROTOCOL_SERVER_HELLO_EVENT);
}
...
std::string EteacherMqttProtocol::DecodeHexString(const std::string& hex_string) {
    std::string decoded;
...
bool EteacherMqttProtocol::IsAudioChannelOpened() const {

...
void SsidManager::AddSsid(const std::string& ssid, const std::string& password) {
    for (size_t i = 0; i < ssid_list_.size(); ++i) {
        auto& item = ssid_list_[i];
        ESP_LOGI(TAG, "compare [%s:%d] [%s:%d]", item.ssid.c_str(), item.ssid.size(), ssid.c_str(), ssid.size());
        if (item.ssid == ssid) {
            ESP_LOGW(TAG, "SSID %s already exists, overwrite and set as default", ssid.c_str());
            item.password = password;
            if (i != 0) {
                auto matched = ssid_list_[i];
                ssid_list_.erase(ssid_list_.begin() + static_cast<std::ptrdiff_t>(i));
                ssid_list_.insert(ssid_list_.begin(), std::move(matched));
            }
            SaveToNvs();
            return;
        }

...
void WifiStation::HandleScanResult() {
    ...
    // Build candidate queue by saved SSID priority (NVS order), not by RSSI.
    // This keeps the default/most recently selected network stable.
    for (const auto& saved : ssid_list) {
        auto* best_match = static_cast<wifi_ap_record_t*>(nullptr);
        for (int i = 0; i < ap_num; i++) {
            auto& ap_record = ap_records[i];
            if (strcmp(reinterpret_cast<const char*>(ap_record.ssid), saved.ssid.c_str()) != 0) {
                continue;
            }
            if (best_match == nullptr || ap_record.rssi > best_match->rssi) {
                best_match = &ap_record;
            }
        }

        if (best_match != nullptr) {
            ESP_LOGI(TAG, "Found AP: %s, BSSID: %02x:%02x:%02x:%02x:%02x:%02x, RSSI: %d, Channel: %d, Authmode: %d",
                reinterpret_cast<const char*>(best_match->ssid), 
                best_match->bssid[0], best_match->bssid[1], best_match->bssid[2],
                best_match->bssid[3], best_match->bssid[4], best_match->bssid[5],
                best_match->rssi, best_match->primary, best_match->authmode);
            WifiApRecord record = {
                .ssid = saved.ssid,
                .password = saved.password,
                .channel = best_match->primary,
                .authmode = best_match->authmode,
                .bssid = {0}
            };
            memcpy(record.bssid, best_match->bssid, 6);
...
void WifiStation::SetScanResultConsumeEnabled(bool enabled) {
    consume_scan_result_ = enabled;
    ESP_LOGI(TAG, "Set scan result consume: %s", consume_scan_result_ ? "enabled" : "disabled");
}
...
void WifiStation::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    ...
    if (event_id == WIFI_EVENT_STA_START) {
        if (!this_->consume_scan_result_) {
            ESP_LOGI(TAG, "Skip auto scan on STA_START (manual app scan mode)");
            return;
        }
        esp_wifi_scan_start(nullptr, false);
        ...
        if (!this_->consume_scan_result_) {
            ESP_LOGI(TAG, "Skip station scan result consume (manual app scan mode)");
            return;
        }
        ...
        if (!this_->consume_scan_result_) {
            ESP_LOGI(TAG, "Skip auto reconnect on disconnect (manual app scan mode)");
            return;
        }
